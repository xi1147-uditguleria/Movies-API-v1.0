...
class MovieControllerTest {
  ...
  void getAllMoviesTest() throws Exception
  {
    ...
    this.mockMvc.perform(MockMvcRequestBuilders.get("/api/v1.0/movies")).andDo(print()).andExpect(status().isOk())
  ...
  void getMovieByIdTest() throws Exception {
    ...
    this.mockMvc.perform(MockMvcRequestBuilders.get("/api/v1.0/movies/3")).andDo(print()).andExpect(status().isOk())
  ...
  void updateMovieByIdTest() throws JsonProcessingException, Exception {
    ...
    ObjectMapper mapper = new ObjectMapper();
    this.mockMvc
            .perform(MockMvcRequestBuilders.put("/api/v1.0/movies").content(mapper.writeValueAsString(request))
                    .contentType(MediaType.APPLICATION_JSON_VALUE))
